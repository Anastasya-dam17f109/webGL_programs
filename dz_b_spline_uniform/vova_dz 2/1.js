...
const Data = {
	...
	calculateApproxSpline: function () {
        ...
        let t, x, y, h, x_t, y_t, h_t,  x_tt, y_tt, h_tt, dx,dy,l;
        //генерация вектора узлов
		let knot_vector = [];
		const N = eval(this.countSplinePoints.value);
		const p = eval(this.splineDegree.value);
		...
		for (i = 0; i< eval(n)+eval(p)+2; i++){
			...
			if(i<=p)
				...
				if (i>n)
					knot_vector.push(n-p+1)
				else{
					for (j = 0; j< i-p-1; j++){
						l+=c_k[j]
					}
					knot_vector.push(((l+((i-p)/(n-p+1))*c_k[i-p])/ck_sum)*(n-p+1))
				}
			}
		}
		let basN   = [];
        ...
        while (t<knot_vector[n+1]){
		    if( flag!=0){
				...
				if (delta_t > t_max)
					delta_t = t_max;
				if (t_res + delta_t > knot_vector[n+1])
					t_res = knot_vector[n+1] + 0.0
				else 
					t_res = t_res + delta_t
				
				t += delta_t; 
			}				
			k = findSpan(knot_vector.length-p-2, p, t_res, knot_vector);

            basisFunc(k,t_res,p,knot_vector,basN);
			x = 0
			y = 0
			h = 0
			for(i = 0; i< p+1; i++){
